...
// Function to Poll for Updated Fitness Data (Checks for Timestamp Change)
const fetchUpdatedFitnessData = async (userId, oldTimestamp = null, maxRetries = 5, delayMs = 2000) => {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const latestData = await fetchLatestFitnessData(userId);

            if (latestData && latestData.timestamp !== oldTimestamp) {
                console.log(`Updated data found after ${attempt} attempt(s)`);
                return latestData;
            }

            console.log(`⏳ Attempt ${attempt}: Waiting for updated fitness data...`);
            await new Promise(resolve => setTimeout(resolve, delayMs)); // Wait before retrying
        } catch (error) {
            console.error("Error fetching updated fitness data:", error);
            throw error;
        }
    }
...
router.get("/profile", async (req, res) => {
    ...
    try {
        // Fetch latest fitness data (if any)
        const initialData = await fetchLatestFitnessData(userId);
        const oldTimestamp = initialData ? initialData.timestamp : null;

        console.log(initialData ? "🔄 Existing fitness data found. Waiting for update..." : "🆕 No data found. Waiting for first-time update...");

        // Poll for latest updated data
        const updatedData = await fetchUpdatedFitnessData(userId, oldTimestamp);
